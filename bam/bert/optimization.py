...
def create_optimizer(config, loss, num_train_steps):
  ...
  learning_rate = tf.constant(value=config.learning_rate, shape=[],
                              dtype=tf.float32)
  ...
  if num_warmup_steps := int(num_train_steps * config.warmup_proportion):
...
def get_key_to_depths(n_layers):
  ...
  for layer in range(n_layers):
    key_to_depths[f"/layer_{str(layer)}/"] = layer + 1
